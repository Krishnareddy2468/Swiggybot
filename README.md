# Swiggy Order Bot — AI Restaurant Ordering Assistant
...
This project implements a conversational AI bot that automates food ordering from Swiggy. Instead of navigating the app manually, users can simply chat with the bot in plain English — search for restaurants, browse menus, add items to cart, and place orders.

The bot is accessible through two channels:
- **Web Chat** — A Next.js frontend with a modern dark-themed UI
- **Telegram** — Search for `@Swiggyfoodbot` on Telegram

Since Swiggy doesn't provide a public API, the project uses a **Mock Swiggy MCP Server** with 10 real-style restaurants and 150+ menu items across Hyderabad locations.
...
```
                    ...
                    +------+----------+----------+--------------+
                           |          |          |
                    +------v---+ +----v----+ +---v--------------+
                    |In-Memory | |JSON File| |Mock Swiggy Data  |
...
│       └── mock_data/
│           └── swiggy_data.py     # 10 restaurants, 150+ items, Hyderabad
│
├── frontend/
│   ├── app/
...
```bash
git clone https://github.com/Krishnareddy2468/Swiggybot.git
cd Swiggybot
```
...
### Mock Swiggy Integration

Since Swiggy doesn't expose a public API, I built a mock data layer that simulates what a real Swiggy MCP Server would return. It includes:
